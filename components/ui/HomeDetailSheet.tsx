...
export function HomeDetailSheet({ home, isOpen, onClose }: HomeDetailSheetProps) {
  ...
  async function handleDeleteRoom(roomId: string) {
    if (!confirm('Is room ko delete karo?')) return
    try {
      ...
      <div style={{ padding: '16px 20px 32px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 1 }}>{home.city ?? 'No city'}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          >
            <Plus size={13} strokeWidth={2.5} /> Add Room
          </button>
        </div>
        ...
        {showInput && (
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="Bedroom, Kitchen, Almirah..."
              style={{
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text-tertiary)', fontSize: 13 }}>
            ...
            Koi room nahi hai. Abhi add karo!
          </div>
        ) : (
